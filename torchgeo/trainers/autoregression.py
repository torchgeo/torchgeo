...
class AutoregressionTask(BaseTask):
        ...
        **kwargs: dict[str, Any],
        ...
        Args:
            model: Name of the model to use, currently supports 'seq2seq'.
                Defaults to 'seq2seq'.
            input_size: The number of features in the decoder input.
                Defaults to 1.
    ...
    def configure_models(self) -> None:
        ...
        if model == 'seq2seq':
            self.model = Seq2Seq(input_size=input_size, **self.kwargs)
        else:
            raise ValueError(
    ...
    def _shared_step(self, batch: Any, batch_idx: int, stage: str) -> Tensor:
        ...
        Returns:
            The loss tensor.
        """
        past_targets = batch['past_targets']
        future_targets = batch['future_targets']
        y_hat = self(past_targets, future_targets)
        loss: Tensor = self.criterion(y_hat, future_targets)
        ...
        if all(key in batch for key in ['mean', 'std']):
            ...
            future_targets = self._denormalize(future_targets, mean, std)
        # Retrieve the correct metrics based on the stage
        metrics = getattr(self, f'{stage}_metrics', None)
        if metrics:
            y_hat = y_hat.flatten(0, 1)
            future_targets = future_targets.flatten(0, 1)
            metrics(y_hat, future_targets)
